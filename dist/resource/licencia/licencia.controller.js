"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LicenciaController = void 0;
const common_1 = require("@nestjs/common");
const licencia_service_1 = require("./licencia.service");
const create_licencia_dto_1 = require("./dto/create-licencia.dto");
const swagger_1 = require("@nestjs/swagger");
const licencia_entity_1 = require("./entities/licencia.entity");
let LicenciaController = class LicenciaController {
    constructor(licenciaService) {
        this.licenciaService = licenciaService;
    }
    create(createLicenciaDto, database) {
        return this.licenciaService.create(createLicenciaDto, database);
    }
    findOne(id, database) {
        return this.licenciaService.findOne(id, database);
    }
    update(id, updateLicenciaDto, database) {
        return this.licenciaService.update(id, updateLicenciaDto, database);
    }
    remove(id, database) {
        return this.licenciaService.remove(id, database);
    }
    findAll(database) {
        return this.licenciaService.findAll(database);
    }
};
exports.LicenciaController = LicenciaController;
__decorate([
    (0, common_1.Post)(),
    (0, common_1.HttpCode)(201),
    (0, swagger_1.ApiResponse)({ status: 201, description: 'Create a new license', type: licencia_entity_1.LicenciaEntity }),
    __param(0, (0, common_1.Body)()),
    __param(1, (0, common_1.Query)('database')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_licencia_dto_1.CreateLicenciaDto, String]),
    __metadata("design:returntype", void 0)
], LicenciaController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(':id'),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Retrieve a license by ID', type: licencia_entity_1.LicenciaEntity }),
    (0, swagger_1.ApiParam)({ name: 'id', type: String, description: 'License ID' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Query)('database')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], LicenciaController.prototype, "findOne", null);
__decorate([
    (0, common_1.Put)(':id'),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Update a license by ID', type: licencia_entity_1.LicenciaEntity }),
    (0, swagger_1.ApiParam)({ name: 'id', type: String, description: 'License ID' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __param(2, (0, common_1.Query)('database')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, create_licencia_dto_1.CreateLicenciaDto, String]),
    __metadata("design:returntype", void 0)
], LicenciaController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':id'),
    (0, common_1.HttpCode)(204),
    (0, swagger_1.ApiResponse)({ status: 204, description: 'Delete a license by ID' }),
    (0, swagger_1.ApiParam)({ name: 'id', type: String, description: 'License ID' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Query)('database')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], LicenciaController.prototype, "remove", null);
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Retrieve all licenses', type: [licencia_entity_1.LicenciaEntity] }),
    (0, swagger_1.ApiQuery)({ name: 'database', type: String, required: false, description: 'Database (pg or mongo)' }),
    __param(0, (0, common_1.Query)('database')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], LicenciaController.prototype, "findAll", null);
exports.LicenciaController = LicenciaController = __decorate([
    (0, swagger_1.ApiTags)('Licencia'),
    (0, common_1.Controller)('licencia'),
    __metadata("design:paramtypes", [licencia_service_1.LicenciaService])
], LicenciaController);
//# sourceMappingURL=licencia.controller.js.map